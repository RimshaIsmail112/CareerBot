  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        res.json({
          message: "Candidate Sign In Successfully",
          id: candidateLogin["_id"],
          email: candidateLogin["email"],
        });
      } catch (error) {
        console.error(error);
    ...
    try {
      ...
      if (employerRegistration) {
        ...
        return res.json({
          ...
          id: employerRegistration["_id"],
          email: employerRegistration["email"],
        });
      } else {
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        res.json({ message: "Employer Sign In Successfully", employer: employerLogin });
      } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Internal Server Error" });
      }
    ...
    if (!otp || !userId) {
      ...
      if (CandidateOTPVerificationRecords.length <= 0) {
        ...
        if (expiresAt < Date.now()) {
          ...
          return res
            .status(400)
            .json({ error: "Code Has Expired. Please Try Again" });
        } else {
