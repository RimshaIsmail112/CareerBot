...
function Page({ params }) {
  ...
  async function handlePortfolioTab() {
    ...
    const response = await fetch(
      `${HOST}/candidate/getProjects/660a89b1c16089e0e1433a6b`,
      {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    async function fetchJobs() {
      const fetchCandidate = await candidateCardData.find(
        (candidate) => candidate._id === decodeURIComponent(params.id)
      );
  ...
  async function handleMeetings() {
    const response = await fetch(
      ...
      )}&employerEmail=${encodeURIComponent("hamzashafqat098@gmail.com")}`,
      {
        method: "GET",
      }
    );
      ...
      <div className="w-full space-y-8 bg-slate-50 p-8 rounded-lg shadow-lg">
        {candidate && (
          <>
            ...
            <p className="text-center text-slate-950 font-medium italic">
              {candidate.email}
            </p>
                  ...
                  {!portfolio ? (
                    ...
                    <>
                      <div className="flex justify-center sm:justify-start w-full p-5 items-center">
                        <IoMdArrowRoundBack
                          size={32}
                          onClick={() => setPortfolioShow(false)}
                        />
                      </div>
                    ...
                    <div className="py-4">
                      ...
                      <div className="flex justify-center items-center gap-2 text-slate-950 flex-wrap mt-4">
                        {candidate.skills.map((skill, index) => (
                          <Badge
                            key={index}
                            className="bg-slate-900 text-slate-50 font-semibold py-1 px-2 rounded-lg "
                          >
                            {skill}
                          </Badge>
                        ))}
                  ...
                  <div className={"flex flex-col md:flex-row gap-3"}>
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          className={
                            "block cursor-pointer w-full mt-4 bg-blue-500 hover:bg-blue-600 text-slate-50 font-bold py-2 px-4 rounded-lg text-center"
                          }
                        >
                          Upload Result
                        </Button>
