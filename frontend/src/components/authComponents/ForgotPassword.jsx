...
export default function ForgotPassword({ resetFor }) {
  ...
  function gettingEmail(userId) {
    if (
      path.includes("/candidate/reset-password") ||
      path.includes("/employer/reset-password")
    ) {
      const candidateGetEndPoint = `${HOST}/candidate/${userId}`;
      const employerGetEndPoint = `${HOST}/employer/${userId}`;
      fetch(
        resetFor === "Candidate" ? candidateGetEndPoint : employerGetEndPoint
      )
        .then((res) => res.json())
        .then(async (data) => {
          if (data.error) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      resetFor === "Candidate"
        ? setCandidate({ id: userId, email })
        : setEmployer({ id: userId, email });
    }
  }, [userId]);
  const onSubmit = (data) => {
    setIsLoading(true);
    ...
    path.includes("/employer/reset-password")
      ? fetch(
          resetFor === "Candidate"
            ? candidateResetEndPoint
            : employerResetEndPoint,
          {
            method: "POST",
            headers: {
      ...
      : fetch(
          resetFor === "Candidate"
            ? candidateForgotEndPoint
            : employerForgotEndPoint,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      <div className="max-w-md w-full h-[60vh] flex flex-col justify-center items-center gap-2 mx-auto rounded-none md:rounded-2xl p-4">
