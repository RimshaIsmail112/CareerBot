...
const Chatbot = ({ question }) => {
  ...
  useEffect(() => {
    async function getResponse() {
      ...
      if (question) {
        ...
        setConversation([
          { text: response, sender: "chatbot", avatar: "/chatbot-avatar.png" },
        ]);
        setIsOpen(true);
        question = null;
      }
  ...
  const handleSendMessage = async () => {
    ...
    const newConversation = [
      ...conversation,
      { text: message, sender: "user", avatar: "/user-avatar.png" },
    ];
  ...
  return (
    <div className="fixed bottom-10 right-10">
      <button
        ...
        className="bg-slate-900 shadow-xl hover:bg-slate-600 text-slate-50 font-bold py-2 px-4 rounded-full focus:outline-none focus:shadow-outline"
      >
        {conversation ? (
      ...
      {isOpen && (
        <motion.div
          ...
          className="w-96 bg-slate-900 p-4 rounded-lg shadow-lg absolute bottom-16 right-0 z-50"
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={toggleChatbot}
              className="text-slate-50 hover:text-slate-300 focus:outline-none"
            >
