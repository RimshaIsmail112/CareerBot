...
const CandidateHeroSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (path.includes("candidate/dashboard")) {
      await router.push(
        `/candidate/dashboard/search-jobs?search=${search}&location=${location}`
      );
      setIsLoading(false);
    } else {
      await router.push(
  ...
  return (
    <div className="pb-12 pt-14 px-10 md:px-12 relative">
      <WavyBackground
        ...
        className="max-w-4xl mx-auto pb-35"
      >
        {path.includes("candidate/dashboard") ? (
          <>
            <h1
              className={`md:text-5xl leading-7 ${
                path.includes("search-jobs") ? "hidden" : ""
              } text-3xl text-center text-slate-50 font-bold mb-7`}
            >
            ...
            <p
              className={`${
                path.includes("search-jobs") ? "hidden" : ""
              } text-slate-50 text-sm text-center`}
            >
          ...
          <>
            ...
            >
              Find Your{" "}
              <span className="text-[#504ED7]">Perfect Candidate</span>{" "}
              <br className="hidden md:block" /> Today!
            </h1>
            <p
              className={`text-slate-50 text-sm text-center ${
                path.includes("search-jobs") ? "hidden" : ""
              }`}
            >
              1,850,750 Profiles Available! Your ideal hire is just a click away
            </p>
          </>
        )}
        <div className="w-full max-w-[800px] m-auto bg-white shadow-xl border border-gray-200 md:rounded-full rounded-md md:h-16 h-auto md:py-0 py-6 px-4 mt-12">
          <form className="flex md:flex-row flex-col justify-between items-center h-full gap-3">
            ...
            <div className="flex w-full items-center gap-3 md:mb-0 mb-5 md:border-none border-b border-slate-700 md:pb-0 pb-3 flex-1">
