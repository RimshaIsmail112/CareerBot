...
function ResumeUploader() {
  ...
  const handleFileInputChange = (e) => {
    ...
    if (file && !allowedFormats.includes(file.type)) {
      setErrorMessage(
        "Invalid file format. Only PDF, DOC, PNG, and JPEG are allowed."
      );
  ...
  const handleFileSubmission = async (e) => {
    ...
    try {
      ...
      await fetch("https://api.cloudinary.com/v1_1/dy5yzo1ji/upload", {
        ...
        .then(async (data) => {
          setErrorMessage("");
          const message = await storeCompleteData(
            candidate.id,
            data.bytes,
            data.format,
            data.secure_url
  ...
  return (
    <BackgroundGradient className="rounded-3xl flex justify-center items-center p-4 h-full w-[80vw] md:w-[40vw] lg:w-[30vw] bg-slate-900">
      {showAlert && <DynamicAlert title={title} alertMessage={alertMessage} />}
      <div
        className={`bg-transparent text-center rounded w-full px-3 flex flex-col items-center justify-center cursor-pointer mx-auto ${
          selectedFile
            ? "border-2 border-slate-200 border-dashed rounded-xl"
            : ""
        }`}
      ...
      >
        <div className="py-6 flex w-full flex-col justify-center items-center gap-3 ">
          {selectedFile ? (
            <MdInsertDriveFile className="text-slate-50" size={50} /> // Display selected file icon
          ) : (
          ...
          {selectedFile ? (
            ...
            <h4 className="text-base font-semibold text-slate-50">
              Drag and drop or select a resume
            </h4>
          )}
        </div>
        ...
        <div className="py-6 w-full">
          {selectedFile ? (
            ...
            <div>
              ...
              <label
                htmlFor="uploadFile1"
                className="bg-slate-50 text-[1rem] font-medium flex justify-center items-center gap-1 dark:bg-zinc-800 w-full text-slate-950 rounded-md h-10 cursor-pointer transition-all duration-300 transform active:bg-slate-900 hover:bg-slate-900 hover:border-slate-50 hover:border-2 hover:text-slate-50"
              >
                Upload Resume
                <MdOutlineUploadFile size={20} />
              </label>
              {errorMessage ? (
                ...
                <p className="text-xs text-gray-400 mt-4">
                  PDF, DOC, PNG, and JPEG are Allowed.
                </p>
              )}
            </div>
          )}
