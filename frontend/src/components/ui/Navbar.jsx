  ...
  return (
    <Menu open={isMenuOpen} handler={setIsMenuOpen} placement="bottom-end">
      ...
      <MenuList className="p-1">
        {profileMenuItems &&
          profileMenuItems.map(({ label, href, icon }, key) => {
            const isLastItem = key === profileMenuItems.length - 1;
            return (
              <MenuItem
                key={label}
                onClick={closeMenu}
                className={`flex items-center p-0 bg-opacity-100 text-slate-950 gap-2 rounded "hover:bg-slate-900 focus:bg-slate-900 active:bg-slate-900 hover:text-slate-50" ${
                  isLastItem
              ...
              >
                <Link
                  ...
                  target={"_self"}
                  className={
                    "flex gap-1 p-2 text-slate-950 hover:text-slate-50 w-full"
                  }
                >
                  {icon}
                  {label}
          ...
          >
            <MenuItem
              className={`flex items-center gap-2 ${
                path.includes(href)
                  ? "bg-slate-900 text-slate-50 bg-opacity-100"
                  : ""
              } hover:bg-slate-900 hover:bg-opacity-100 hover:text-slate-50 lg:rounded-full`}
            >
              {icon}
              <span> {label}</span>
            </MenuItem>
