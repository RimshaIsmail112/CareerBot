...
export const JobCard = ({
  ...
  return (
    <div className="card-wrapper bg-slate-50 cursor-pointer rounded-xl shadow-black shadow-lg hover:shadow-xl transition duration-200 p-6 flex flex-col gap-3">
      <div className="self-end">
        <div className="flex justify-center items-center gap-2">
          <JobBadge data={{ location, country }} isLocation />
          {isBookmarked ? (
            <FaBookmark onClick={handleJobBookMark} size={20} />
          ) : (
            ...
            <div className="flex flex-col-reverse items-start justify-between gap-5 sm:flex-row">
              <div className="flex-1">
                {employerLogo ? (
                  ...
                  <div className="mb-6 sm:hidden w-20 h-20 bg-slate-900 flex items-center justify-center">
                    <span className="text-slate-50 text-2xl">
                      {employerName.slice(0, 2).toUpperCase()}
                    </span>
                  </div>
    ...
    <div className="card-wrapper bg-slate-50 cursor-pointer rounded-xl shadow-black shadow-lg hover:shadow-xl transition duration-200 p-6 flex flex-col gap-3">
      <div>
        <div className="flex justify-between items-center gap-2">
          <JobBadge data={{ location, country }} isLocation />
          {isBookmarked ? (
            <FaBookmark onClick={handleBookMark} size={20} />
          ) : (
      ...
      <Link
        href={`/employer/candidate/${id}`}
        className="flex flex-col justify-center items-center gap-4"
      >
        <div className="block">
          <JobBadge
            className={"w-40 h-40 md:w-36 md:h-36 rounded-full"}
            data={{ logo: imageUrl }}
            isCandidate
          />
        </div>
